...
// Node 22.5+ provides a durable, local SQLite implementation.
// @ts-ignore
...
function limited(ip: string) {
  const now=Date.now(); const recent=(hits.get(ip)||[]).filter(x=>now-x<WINDOW_MS);
...
export function createServerForDb(db: any) {
  ...
  return createServer(async (req,res)=>{
    ...
    if (req.url==="/api/v1/checks" && req.method==="POST") {
      try {
        const input=validate(await body(req)); const id=randomUUID(); const createdAt=new Date().toISOString();
        db.prepare("INSERT INTO checks(id,payload,created_at) VALUES(?,?,?)").run(id,JSON.stringify(input),createdAt);
        return send(res,201,{id,createdAt,check:input});
      } catch (error:any) {
    ...
    if (match && req.method==="GET") {
      const row=db.prepare("SELECT payload,created_at FROM checks WHERE id=?").get(match[1]);
      if (!row) return send(res,404,{error:"check_not_found"});
      return send(res,200,{id:match[1],createdAt:row.created_at,check:JSON.parse(row.payload)});
    }
...
if (process.env.NODE_ENV !== "test") {
  ...
  app.listen(port,"127.0.0.1",()=>console.log(`OfferSignal API listening on http://127.0.0.1:${port}`));
}
